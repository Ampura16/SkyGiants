...
public class Game implements IColorizable {
    ...
    // ========== 构造方法 ==========
    public Game(JavaPlugin plugin, String name, GameConfigManager configManager, ScoreboardManager scoreboardManager) {
    ...
    // ========== 队伍管理方法 ==========
    public boolean createTeam(String name, TeamColor color, int maxPlayers) {
        if (teams.containsKey(name)) {
            return false;
        }
        Team team = new Team(name, color, maxPlayers);
        teams.put(name, team);
        return true;
    }
    ...
    public boolean saveTeamToConfig(String teamName, TeamColor color, int maxPlayers) {
        YamlConfiguration config = configManager.getGameConfig(name);
        if (config == null) {
            return false;
        }
        ...
        // 保存配置文件
        return configManager.saveGameConfig(name, config);
    }
