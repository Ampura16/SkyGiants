    ...
    public boolean execute(CommandSender sender, List<String> args) {
        ...
        if (game.getState() != GameState.WAITING) {
            sender.sendMessage(getColoredMessage("&c游戏 &e%s &c当前状态无法开始 (当前状态: %s)",
                    game.getName(), game.getState().name()));
            return true;
        }
        ...
        if (game.canStart()) {
            sender.sendMessage(getColoredMessage(
                    ...
                    game.getName(), game.getMinPlayers(),
        ...
        if (game.start()) {
            ...
            sender.sendMessage(getColoredMessage("&a成功开始游戏 &e%s", game.getName()));
            return true;
        }

        sender.sendMessage(getColoredMessage("&c无法开始游戏 &e%s", game.getName()));
        return false;
    }
